...
namespace Microsoft.VisualStudio.TestTools.UnitTesting
{
    ...
    public static partial class AssertEx
    {
        ...
        {
            return type.GetTypeInfo().IsPrimitive || type == typeof(string) || type == typeof(char);
        }
        ...
        private class DynamicAccessor<T> : DynamicObject
        {
            ...
            public override bool TrySetIndex(SetIndexBinder binder, object[] indexes, object value)
            {
                try
                {
                    var setProp = typeof(T).GetTypeInfo()
                        .DeclaredProperties
                        .Single(p => p.CanWrite && p.Name == "Item");
                    setProp.SetValue(target, value);
                    return true;
                }
            ...
            public override bool TryGetIndex(GetIndexBinder binder, object[] indexes, out object result)
            {
                try
                {
                    var setProp = typeof(T).GetTypeInfo()
                        .DeclaredProperties
                        .Single(p => p.CanRead && p.Name == "Item");
                    result = setProp.GetValue(target);
                    return true;
                }
                catch (MissingMethodException) { throw new ArgumentException(string.Format("indexer not found : Type <{0}>", typeof(T).Name)); };
            }
            ...
            public override bool TryInvokeMember(InvokeMemberBinder binder, object[] args, out object result)
            {
                var csharpBinder = binder.GetType().
                    ...
                    .SingleOrDefault(t => t.FullName == "Microsoft.CSharp.RuntimeBinder.ICSharpInvokeOrInvokeMemberBinder");
    ...
    public static class TestContextExtensions
    {
        ...
        {
            var type = typeof(TestContextExtensions).GetTypeInfo().Assembly.GetType(context.FullyQualifiedTestClassName);
